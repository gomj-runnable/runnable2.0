...
├── {ApiC}OriginalResponse       ← class: API-C 원본 (3개 이상 가능)
...
├── {sourceC}.adapter.ts         ← API-C 호출 + 원본 → 공통 변환
...
app/composables/
├── sideeffect/use{Domain}Sideeffect.ts  ← API 호출 + withErrorBoundary 래핑
...
## withErrorBoundary 활용

프론트엔드 sideeffect에서 통합 API를 호출할 때 `withErrorBoundary`로 래핑하여 실패를 격리한다:

```typescript
import { withErrorBoundary } from '~/composables/action/useAsyncDecorator'

const fetchWeather = withErrorBoundary(
    () => $fetch('/api/weather/hourly'),
    { retry: 1, fallback: [] }
)
```

## 예시 참조 — Weather 3+1개 API 통합

| Adapter | API 소스 | 역할 |
|---------|----------|------|
| `observed.adapter.ts` | 기상청 관측 API | 현재 관측값 → `HourlyWeather` |
| `forecast.adapter.ts` | 기상청 예보 API | 예보값 → `HourlyWeather` |
| `airquality.adapter.ts` | 대기질 API | 대기질 → `HourlyWeather` (확장) |

### 레퍼런스
- `server/utils/weather/observed.adapter.ts` — 관측 adapter
- `server/utils/weather/forecast.adapter.ts` — 예보 adapter
- `server/utils/weather/airquality.adapter.ts` — 대기질 adapter
- `server/utils/weather/merge.service.ts` — 병합 로직
- `server/utils/weather/weather.service.ts` — 오케스트레이터
...
7. **sideeffect 연결** — `withErrorBoundary`로 래핑하여 실패 격리
...
- sideeffect에서 `withErrorBoundary`로 실패를 격리하는가

## 관련 스킬

- `create-api-service` — 단일 API의 원본 Response Class + Local Response 분리 규칙
- `create-type-role` — 공통 도메인 타입 → adapter 정규화 → 서비스 소비 패턴
