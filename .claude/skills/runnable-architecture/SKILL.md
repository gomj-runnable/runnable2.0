...
| 다수 sideeffect/store를 조합하는 Facade | `app/composables/useRouteMapFacade.ts` (루트 직속) |
...
| Auth Service DI | `server/utils/auth.service.ts` (IAuthService + Memory/BetterAuth 구현체) |
...
| 패턴 | 적용 위치 예시 |
|------|---------------|
| Decorator | `app/composables/action/useAsyncDecorator.ts` — 비동기 함수에 retry/fallback 래핑 |
| StateMachine | `app/composables/action/usePlaybackStateMachine.ts` — 상태 전이 규칙 |
| Strategy | `server/utils/routing/` — 경로 최적화 알고리즘 교체 |
...
| Provider | `app/composables/sideeffect/useCesiumRuntime.ts` — 외부 의존성 단일 접근점 |
