...
- Toss 디자인 시스템의 Flat + Compound 병행 원칙에 따라 확장성이 높은 컴포넌트를 만든다.
...
- 새 UI 컴포넌트를 추가할 때
- 기존 사이드바, 패널, 모달, 카드, 버튼 구조를 정리할 때
- 컴포넌트와 CSS 파일 경계를 함께 손봐야 할 때
- props/slot/emit 인터페이스를 다시 설계해야 할 때
...
- `DrawRoutePanel.vue` — 경로 그리기 편집 패널
...
- 여러 컴포넌트가 공유하는 버튼, 카드, 입력, 라벨 골격은 `app/assets/css/components/common.css`에서 관리한다
...
- organization 계열 카드처럼 재사용 UI는 `app/assets/css/components/organization/**`에 둘 수 있다
...
- Vue 컴포넌트는 `app/components/map/**`에 둔다
- 외부 CSS는 `app/assets/css/components/templates/**`, `app/assets/css/components/molecules/**`, `app/assets/css/components/organization/**`처럼 역할별로 둔다
- `style src`는 실제 파일 경로와 반드시 일치시킨다
- 구조 변경 후에는 더 이상 쓰지 않는 class, CSS variable override, 가상 구분선 같은 잔재를 바로 정리한다
...
- 중복되는 버튼/입력/카드 골격이 `common.css` 또는 공용 card CSS로 통합 가능한 상태인가
