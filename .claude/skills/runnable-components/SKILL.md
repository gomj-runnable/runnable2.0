...
| 계층 | 위치 | 역할 |
|------|------|------|
| atoms | `app/components/<page>/atoms/` | 최소 입력 단위 (Textfield, HoverTooltip 등), 제한적 사용 |
| molecules | `app/components/<page>/molecules/` | 재사용 최소 UI. props 입력, slot 내용 교체, emit 인터랙션 전달. 외부 상태·composable 의존 금지 |
| organizations | `app/components/<page>/organizations/` | 재사용 가능한 복합 UI 블록 (Card 등). molecules를 조합하되 독립적인 데이터 단위를 표현 |
| templates | `app/components/<page>/templates/` | molecules/organizations를 slot으로 조합하는 레이아웃. 내용 직접 렌더링 금지, 최소 props만 허용 |

### 레퍼런스
- `app/components/map/atoms/` — HoverTooltip 등 최소 단위
- `app/components/map/molecules/` — BottomDrawer, RouteInfoPanel 등
- `app/components/map/organizations/cards/` — Card, RouteDiscoverCard, TextfieldCard
- `app/components/map/templates/` — MapShell, PopupModal, SimulationDrawer
...
1. 페이지 결정 → 2. atoms/molecule/organization/template 판단 → 3. Flat/Compound 결정 → 4. props→slot→emit 설계 → 5. 외부 CSS 작성 + `style src` 연결 → 6. semantic token 참조 → 7. 페이지에서 template 조합

## 점검 항목

- molecule이 composable·전역 상태에 의존하지 않는가
- organization이 독립적인 데이터 단위를 표현하는가
