...
- Overlay 자체 상태(열림/닫힘, 활성 레이어 등)는 해당 슬라이스의 `model/`에서 관리
- 내부 sub-modal(달력 팝업 등)은 Overlay 내 `ref`로 로컬 관리
- 외부 데이터 fetch는 해당 슬라이스의 `api/`에 위임
- 데이터 변환은 해당 슬라이스의 `lib/`에 위임
...
2. **Vue 컴포넌트 생성** — `app/features/{name}/ui/{Name}Overlay.vue` 또는 `app/widgets/{name}/ui/{Name}Overlay.vue`
