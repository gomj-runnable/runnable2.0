...
| 3 | `server/services` + `server/repositories` | 중간 (Testcontainers 패턴 재사용) |
...
3. **Nitro 테스트 패턴 PoC 먼저** — 첫 1개 핸들러로 \`@nuxt/test-utils\` 의 \`setup({ server: true })\` 또는 핸들러 함수 직접 호출 패턴 정립. \`server/test/setup.ts\` 의 h3 stub 활용.
