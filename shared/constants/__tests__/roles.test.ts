...
describe('ROLES', () => {
    it('USER < SUPPORT < QA < MODERATOR < ANALYST < ADMIN < DEVELOPER 위계 유지', () => {
        expect(ROLES.USER).toBe(1)
        expect(ROLES.SUPPORT).toBe(10)
        expect(ROLES.QA).toBe(20)
        expect(ROLES.MODERATOR).toBe(30)
        ...
        expect(ROLES.SUPPORT).toBeLessThan(ROLES.QA)
        expect(ROLES.QA).toBeLessThan(ROLES.MODERATOR)
        expect(ROLES.MODERATOR).toBeLessThan(ROLES.ANALYST)
