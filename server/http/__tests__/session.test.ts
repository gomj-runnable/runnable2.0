...
describe('session helpers', () => {
    ...
    it('getSessionUser 는 authService.getSession 위임', async () => {
        vi.mocked(authService.getSession).mockResolvedValue(owner)
        await expect(getSessionUser(event)).resolves.toBe(owner)
        expect(authService.getSession).toHaveBeenCalledWith(event)
    })

    it('requireSession 은 authService.requireSession 위임', async () => {
        vi.mocked(authService.requireSession).mockResolvedValue(owner)
